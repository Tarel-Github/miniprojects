...
class qtApp(QMainWindow):
    ...
    def btnSaveClicked(self): # 저장
        ...
        if fullName == '' or phoneNum == '' :
            QMessageBox.warning(self, '주의', '이름과 폰번을 입력하세요')
            return # 진행불가
        else:
            self.conn = pymysql.connect(host='localhost', user ='root', password='12345', db = 'miniproject', charset='utf8')
            if self.curIdx == 0:
            ...
            if self.curIdx == 0:
                ...
                QMessageBox.about(self, '성공', '데이터 변경 성공')
    ...
    def makeTable(self, rows):
        ...
        self.tblAddress.setColumnWidth(2, 105) # 현드폰 열 105
        self.tblAddress.setColumnWidth(3, 175) # 이메일 열
        self.tblAddress.setColumnWidth(4, 200) # 주소 열
        self.tblAddress.setEditTriggers(QAbstractItemView.NoEditTriggers) # 1. 칼럼수정금지
